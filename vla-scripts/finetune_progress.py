...
#   --wandb_project "CycleVLA_libero_sub_decomposed_stop_oversample_oft_A10" \
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if should_log_l1_loss:
            ...
            # Detailed L1s - now handle both stop and progress
            curr_action_l1_action = torch.nn.L1Loss()(ground_truth_curr_action[:, :7], predicted_curr_action[:, :7])
            curr_action_l1_stop = torch.nn.L1Loss()(ground_truth_curr_action[:, 7:8], predicted_curr_action[:, 7:8])  # Binary stop
            curr_action_l1_progress = torch.nn.L1Loss()(ground_truth_curr_action[:, 8:9], predicted_curr_action[:, 8:9])  # Progress

            next_action_l1_action = torch.nn.L1Loss()(ground_truth_next_actions[:, :, :7], predicted_next_actions[:, :, :7])
            next_action_l1_stop = torch.nn.L1Loss()(ground_truth_next_actions[:, :, 7:8], predicted_next_actions[:, :, 7:8])
            next_action_l1_progress = torch.nn.L1Loss()(ground_truth_next_actions[:, :, 8:9], predicted_next_actions[:, :, 8:9])
            ...
            metrics.update({
                ...
                "curr_action_l1_loss_progress": curr_action_l1_progress.item(), 
                "next_actions_l1_loss_action": next_action_l1_action.item(),
                "next_actions_l1_loss_stop": next_action_l1_stop.item(),
                "next_actions_l1_loss_progress": next_action_l1_progress.item(),
            })
...
def finetune(cfg: FinetuneConfig, stop: bool = True) -> None:
    ...
    if stop:
        recent_metrics = {
            ...
            "curr_action_l1_loss_action": deque(maxlen=cfg.grad_accumulation_steps),
            "curr_action_l1_loss_stop": deque(maxlen=cfg.grad_accumulation_steps),
            "curr_action_l1_loss_progress": deque(maxlen=cfg.grad_accumulation_steps),
            "next_actions_accuracy": deque(maxlen=cfg.grad_accumulation_steps),
            "next_actions_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
            "next_actions_l1_loss_action": deque(maxlen=cfg.grad_accumulation_steps),
            "next_actions_l1_loss_stop": deque(maxlen=cfg.grad_accumulation_steps),
            "next_actions_l1_loss_progress": deque(maxlen=cfg.grad_accumulation_steps), 
        }
    else:
        recent_metrics = {
            "loss_value": deque(maxlen=cfg.grad_accumulation_steps),
