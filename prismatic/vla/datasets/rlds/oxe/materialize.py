...
def make_oxe_dataset_kwargs(
    ...
    # Normalize all action dimensions *except* the gripper, stop, and progress signal
    if dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
        # 6 DoF + gripper + stop + progress
        dataset_kwargs["absolute_action_mask"] = [False] * 6 + [True] + [True] + [True]  # new length = 9
        dataset_kwargs["action_normalization_mask"] = [True] * 6 + [False] + [False] + [False]
    elif dataset_kwargs["action_encoding"] is ActionEncoding.EEF_R6:
        # 9D rot6 + gripper + stop + progress
        dataset_kwargs["absolute_action_mask"] = [False] * 9 + [True] + [True] + [True]  # new length = 12
        dataset_kwargs["action_normalization_mask"] = [True] * 9 + [False] + [False] + [False]
...
#     # [Contract] For EEF_POS & EEF_R6 actions, only the last action dimension (gripper) is absolute!
#     # Normalize all action dimensions *except* the gripper and stop signal
#     if dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
#         # 6 DoF + gripper + stop
#         dataset_kwargs["absolute_action_mask"] = [False] * 6 + [True] + [True]  # new length = 8
#         dataset_kwargs["action_normalization_mask"] = [True] * 6 + [False] + [False]
#     elif dataset_kwargs["action_encoding"] is ActionEncoding.EEF_R6:
#         # 9D rot6 + gripper + stop
#         dataset_kwargs["absolute_action_mask"] = [False] * 9 + [True] + [True]  # new length = 11
#         dataset_kwargs["action_normalization_mask"] = [True] * 9 + [False] + [False]
