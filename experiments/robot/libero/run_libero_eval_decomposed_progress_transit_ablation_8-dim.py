"""
run_libero_eval_decomposed_progress_transit_ablation_8-dim.py

9-dim vs 8-dim ablation against the deprecated 8-dim VLA.

Loads the legacy `libero_sub_decomposed_progress_A100_deprecated` checkpoint
where stop and progress are fused into a single channel. With no separate
stop signal, subtask transitions are driven purely by a progress threshold
(`progress_signal >= 0.95`), and only one channel is dropped before stepping
the env (`action[:-1]` vs the 9-dim path's `action[:-2]`). Used to produce
the paper's ablation comparing fused (8-dim) vs split (9-dim) stop/progress
supervision. Otherwise mirrors `..._transit.py`.
"""

# watch -n 1 nvidia-smi
# conda activate /hdd2/chenyang/openvla-oft/env

# CUDA_VISIBLE_DEVICES="2" python experiments/robot/libero/run_libero_eval_decomposed_rule_progress.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b-oft-finetuned-libero-spatial-object-goal-10+libero_decomposed_progress+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state   --task_suite_name libero_spatial
# CUDA_VISIBLE_DEVICES="2" python experiments/robot/libero/run_libero_eval_decomposed_rule_progress.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b-oft-finetuned-libero-spatial-object-goal-10+libero_decomposed_progress+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state   --task_suite_name libero_object
# CUDA_VISIBLE_DEVICES="2" python experiments/robot/libero/run_libero_eval_decomposed_rule_progress.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b-oft-finetuned-libero-spatial-object-goal-10+libero_decomposed_progress+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state   --task_suite_name libero_goal
# CUDA_VISIBLE_DEVICES="2" python experiments/robot/libero/run_libero_eval_decomposed_rule_progress.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b-oft-finetuned-libero-spatial-object-goal-10+libero_decomposed_progress+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state   --task_suite_name libero_10
...
class GenerateConfig:
    ...
    num_trials_per_task: int = 5                     # Number of rollouts per task (50)
    ...
    local_log_dir: str = "./experiments/logs/logs_sub_decomposed_rule_progress_A100_8-dim_30000_chkpt"        # Local directory for eval logs
    video_save_dir: str = "./rollouts/rollouts_sub_decomposed_rule_progress_A100_8-dim_30000_chkpt"
...
def run_episode(
    ...
    # subtask_list = [f"The current subtask: {state}" for state in states]
    # states = [states[-1]] * len(states)
    # subtask_list = [subtask_list[-1]] * len(subtask_list)
    # subtask_list = subtask_list[::-1]
    ...
    try:
        for subtask_idx, (current_subtask, current_state) in enumerate(zip(subtask_list, states)):
            ...
            # while t < max_steps // len(states) + cfg.num_steps_wait:
            while t < max_steps * 2 + cfg.num_steps_wait:
                if t == 0:
                ...
                action = process_action(action_queue.popleft(), cfg.model_family, stop=False, progress=True)
                print(action)

                # Step environment
                progress_signal = action[-1]  # Now 0.0 to 1.0 progress signal
                ...
                if progress_signal >= progress_threshold:
                    ...
                    if not first_high_seen:
                        ...
                        log_message(f"First high progress signal {progress_signal:.2f} at step {t} for subtask: {current_state}", log_file)
                    
                    # Break conditions
                    if consecutive_high_count >= 2:
                        # Consecutive highs - immediate confirmation
                        log_message(f"Finished subtask: {current_state} at step {t} ({consecutive_high_count} consecutive high signals, latest={progress_signal:.2f}).", log_file)
                        break
                    elif first_high_seen and steps_since_last_high >= 2:
                        # Confirmation after doubt - high after 2+ low steps
                        log_message(f"Finished subtask: {current_state} at step {t} (progress={progress_signal:.2f} after {steps_since_last_high} low-progress steps).", log_file)
                        break
                    else:
                        if first_high_seen and steps_since_last_high > 0:
                            log_message(f"Ignoring progress signal {progress_signal:.2f} at step {t} (only {steps_since_last_high} low steps since last high, need 2+).", log_file)
                    
                    # Reset the low-signal counter
                    steps_since_last_high = 0
                    
                else:  # Low progress signal
                    consecutive_high_count = 0  # Reset consecutive counter
                    if first_high_seen:
