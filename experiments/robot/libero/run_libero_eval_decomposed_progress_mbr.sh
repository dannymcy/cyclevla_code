...
# chmod +x experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.sh
# ./experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.sh
...
for i in "${!CHECKPOINT_NUMS[@]}"; do
    ...
    LOCAL_LOG_DIR="./experiments/logs/logs_sub_decomposed_progress_mbr_${CHKPT_NUM}_chkpt_away"
    VIDEO_SAVE_DIR="./rollouts/rollouts_sub_decomposed_progress_mbr_${CHKPT_NUM}_chkpt_away"
    VIDEO_BASE_DIR="./rollouts/rollouts_sub_decomposed_progress_transit_${CHKPT_NUM}_chkpt"

    echo "============================================="
    echo "Processing checkpoint: ${CHKPT_NUM}"
    echo "Num trials per task: ${NUM_TRIALS}"
    echo "Video base dir: ${VIDEO_BASE_DIR}"
    echo "============================================="

    # Run each task sequentially
    for TASK in "${TASK_SUITES[@]}"; do
        echo "Running task: $TASK"
        CUDA_VISIBLE_DEVICES="2" python experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.py \
