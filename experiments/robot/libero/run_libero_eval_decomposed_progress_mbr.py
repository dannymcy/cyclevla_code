"""
run_libero_eval_decomposed_progress_mbr.py

Official CycleVLA evaluation pipeline (paper's final eval).

Uses the 9-dim subtask-aware policy to step through subtasks via the stop +
progress signals. At ~90% progress per subtask, a VLM is queried with
`type: transit | backtrack`; on `backtrack` the env is physically rewound to
the start of the failing subtask, N seeds are resampled, and the best
trajectory is selected via Minimum Bayes Risk (MBR). Per-component latencies
(`vlm_detector`, `action_inference`, `sampling`, `mbr_computation`,
`backtracking`) are tracked.

Loads checkpoints from `checkpoints/libero/libero_sub_decomposed_progress_A100/`.
See sibling scripts for stripped-down variants:
  - `..._transit.py`        — same policy, no VLM / MBR / backtrack
  - `..._transit_seed.py`   — `..._transit` + per-seed trajectory dumper
                              feeding `run_mbr_analysis.py`
  - `..._transit_ablation_8-dim.py` — 8-dim (deprecated) ablation
"""

# watch -n 1 nvidia-smi
# conda activate /hdd2/chenyang/openvla-oft/env

# CUDA_VISIBLE_DEVICES="3" python experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b+libero_decomposed_progress+b16+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state--50000_chkpt   --task_suite_name libero_spatial
# CUDA_VISIBLE_DEVICES="3" python experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b+libero_decomposed_progress+b16+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state--50000_chkpt   --task_suite_name libero_object
# CUDA_VISIBLE_DEVICES="3" python experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b+libero_decomposed_progress+b16+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state--50000_chkpt   --task_suite_name libero_goal
# CUDA_VISIBLE_DEVICES="3" python experiments/robot/libero/run_libero_eval_decomposed_progress_mbr.py   --pretrained_checkpoint /hdd2/chenyang/openvla-oft/checkpoints/libero/libero_sub_decomposed_progress_A100/openvla-7b+libero_decomposed_progress+b16+lr-0.0005+lora-r32+dropout-0.0--image_aug--parallel_dec--8_acts_chunk--continuous_acts--diffusion--3rd_person_img--wrist_img--proprio_state--50000_chkpt   --task_suite_name libero_10
...
class VLMDetector:
    def __init__(self, model_name="gpt-5.2", temperature=1):
...
class GenerateConfig:
    ...
    local_log_dir: str = "./experiments/logs/logs_sub_decomposed_progress_mbr_50000"        # Local directory for eval logs
    video_save_dir: str = "./rollouts/rollouts_sub_decomposed_progress_mbr_50000"
    video_base_dir: str = "./rollouts/rollouts_sub_decomposed_progress_transit_50000"
...
def run_episode(
    ...
    vlm_detector = VLMDetector(model_name="gpt-5.2", temperature=1)
...
def get_failed_episodes_from_videos(cfg: GenerateConfig, log_file=None):
    """
    Parse video directory to find FAILED episodes for the current task suite.
    Returns a dictionary in the format: {task_suite_name: [list of failed episode numbers]}
    """
    # Search in the task-specific subdirectory
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        if rerun:
            ...
            save_rollout_video_decomposed(
                ...
                video_save_dir=os.path.join(cfg.video_save_dir, cfg.task_suite_name), log_file=log_file, subtasks=replay_subtasks, wrist=True
            )
